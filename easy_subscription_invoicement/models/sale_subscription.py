...
class SaleSubscription(models.Model):
    ...
    next_payment_date = fields.Date(readonly=True)
    within_payment_date = fields.Boolean(readonly=True, compute="_compute_within_payment_date")
    payment_term_id = fields.Many2one('account.payment.term', string='Payment Terms',
        domain="['|', ('company_id', '=', False), ('company_id', '=', company_id)]")

    def __compute_within_payment_date(self):
        today = datetime.date.today()
        for subscription in self:
            if subscription.next_payment_date.to_date() <= today <= subscription.recurring_next_date.to_date():
                subscription.within_payment_date = True

    @api.onchange("recurring_next_date")
    def compute_next_payment_date(self):
        interval = timedelta(days=15)
        for subscription in self:
            if subscription.recurring_next_date:
                subscription.next_payment_date = subscription.recurring_next_date - interval

    def action_calculate_next_payment_date(self):
        subscription_model = self.env["sale.subscription"]
        subscriptions = subscription_model.search([])
        for subscription in subscriptions:
            subscription.compute_next_payment_date()

    def _prepare_invoice(self):
        invoice = self._prepare_invoice_data()
        if self.partner_id.payment_policy:
            invoice["payment_policy"] = self.partner_id.payment_policy

        if self.partner_id.usage:
            invoice["l10n_mx_edi_usage"] = self.partner_id.usage

        if self.partner_id.method_id:
            invoice["l10n_mx_edi_payment_method_id"] = self.partner_id.method_id.id
                ...
                for subscription in subs:
                        ...
                        try:
                            ...
                            if payment_token:
                                ...
                                if tx.state in ['done', 'authorized']:
                                    ...
                                    msg_body = _(
                                        'Automatic payment succeeded. Payment reference: <a href=# data-oe-model=payment.transaction data-oe-id=%d>%s</a>; Amount: %s. Invoice <a href=# data-oe-model=account.invoice data-oe-id=%d>View Invoice</a>.') % (
                    ...
                    # invoice only
                        ...
                        try:
                            ...
                            # invoicing_period = relativedelta(months=1)
                            ...
                            if automatic and auto_commit:
                                ...
                                #raise ValidationError(subscription.code)
                            else:
                                raise
        return invoices
